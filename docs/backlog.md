...
- [x] El correo de bienvenida de esa alta falló en Render con `Connection timeout`: el plan gratuito bloquea el SMTP saliente. Resuelto sin cuentas nuevas con `EMAIL_TRANSPORT=foodzinder`, que envía por el relevo `POST /api/v1/admin/email` añadido al P4 (nota en ADR-0002).
